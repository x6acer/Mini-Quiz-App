...
function startTimer() {
    ...
    timer = setInterval(() => {
        ...
        if (remaining <= 0) {
            ...
            timeLeftArray[currentQuestion] = remaining;
...
function autoSubmit() {
    ...
    if (selected && userAnswers[currentQuestion] === null) {
        userAnswers[currentQuestion] = parseInt(selected.value);
        let isCorrect = userAnswers[currentQuestion] === questions[currentQuestion].correct;
        result.textContent = isCorrect
            ? "Time's up! ✅ Correct!"
            : `Time's up! ❌ The correct answer is "${questions[currentQuestion].answers[questions[currentQuestion].correct]}".`;
        result.style.fontWeight = "bold";
        if (isCorrect) score++;
        scores.textContent = `Score: ${score}`;
    } else if (!selected && userAnswers[currentQuestion] === null) {
